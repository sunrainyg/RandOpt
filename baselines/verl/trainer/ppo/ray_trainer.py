...
class RayPPOTrainer:
    ...
    def _dump_generations(self, inputs, outputs, gts, scores, reward_extra_infos_dict, dump_path):
        ...
        # --- Dump categorized ID summary ---
        # IDs are integer dataset indices (0, 1, 2, ...) corresponding to row order in the parquet file.
        # Categories:
        #   correct_ids:       score >= 1.0 (answered correctly, i.e. full score)
        #   incorrect_ids:     score < 1.0 (answered incorrectly, includes format-only 0.1 and 0)
        #   strict_format_ids: has_strict_format == 1 (output contains "#### <number>")
        #   no_strict_format_ids: has_strict_format == 0 (output does NOT contain "#### <number>")
        has_strict_format_list = reward_extra_infos_dict.get("has_strict_format", [])
        if len(has_strict_format_list) == n:
            correct_ids = []
            incorrect_ids = []
            strict_format_ids = []
            no_strict_format_ids = []

            for i in range(n):
                # Use integer index as dataset row ID (matches parquet row order)
                sample_id = i

                # Correct vs incorrect: score >= 1.0 means fully correct answer
                if scores[i] >= 1.0:
                    correct_ids.append(sample_id)
                else:
                    incorrect_ids.append(sample_id)

                # Strict format compliance (has "#### <number>")
                if has_strict_format_list[i] == 1:
                    strict_format_ids.append(sample_id)
                else:
                    no_strict_format_ids.append(sample_id)
            ...
            summary = {
                ...
                "num_strict_format": len(strict_format_ids),
                "num_no_strict_format": len(no_strict_format_ids),
                "correct_ids": correct_ids,
                "incorrect_ids": incorrect_ids,
                "strict_format_ids": strict_format_ids,
                "no_strict_format_ids": no_strict_format_ids,
            }

            summary_filename = os.path.join(dump_path, f"{self.global_steps}_id_summary.json")
            with open(summary_filename, "w") as f:
                json.dump(summary, f, ensure_ascii=False, indent=2)

            print(f"Dumped ID summary to {summary_filename}")
            print(f"  Correct (flexible): {len(correct_ids)}/{n} ({len(correct_ids)/n*100:.1f}%)")
            print(f"  Incorrect (flexible): {len(incorrect_ids)}/{n} ({len(incorrect_ids)/n*100:.1f}%)")
            print(f"  Strict format (####): {len(strict_format_ids)}/{n} ({len(strict_format_ids)/n*100:.1f}%)")
            print(f"  No strict format: {len(no_strict_format_ids)}/{n} ({len(no_strict_format_ids)/n*100:.1f}%)")
    ...
    def fit(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.get("val_before_train", True):
            ...
            if self.config.trainer.get("val_only", False):
                logger.finish()  # Explicitly finish wandb/other loggers to avoid "crashed" status
                return
