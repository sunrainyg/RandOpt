...
try:
    ...
    # transformers >= 4.54 removed Vision2Seq auto class
...
def load_valuehead_model(local_path, torch_dtype, model_config, trust_remote_code):
    ...
    # Get attn_implementation from model_config if available, otherwise default to flash_attention_2
    attn_impl = getattr(model_config, "_attn_implementation", None) or getattr(model_config, "attn_implementation", "flash_attention_2")
    ...
    # build critic from CausalLM and replace lm_head with a scalar value head.
    try:
        ...
        if not hasattr(model, "lm_head"):
            raise RuntimeError(f"model({local_path}) has no lm_head for critic fallback")
        model.lm_head = nn.Sequential(
            nn.Linear(model.config.hidden_size, 1, dtype=torch_dtype), LambdaLayer(fn=squeeze)
        )
        return model
    except BaseException as causal_fallback_error:
        if not is_trl_available():
            raise RuntimeError(
                f"model({local_path}) failed TOKEN_CLS and CausalLM critic fallback; "
                "install trl for value-head fallback or use a TOKEN_CLS-capable model."
            ) from causal_fallback_error
