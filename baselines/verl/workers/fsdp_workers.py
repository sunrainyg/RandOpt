...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        def _load_actor_module_with_context(context_factory):
            with context_factory(), warnings.catch_warnings():
                warnings.simplefilter("ignore")
                return actor_module_class.from_pretrained(
        ...
        try:
            actor_module = _load_actor_module_with_context(init_context)
        except TypeError as e:
            # transformers>=5 can attach private attrs (for example _is_hf_initialized)
            # that are incompatible with some accelerate meta-init paths.
            if "_is_hf_initialized" not in str(e):
                raise
            logger.warning(
                "Meta tensor init is incompatible with current transformers/accelerate combo; "
                "retrying model load with CPU init."
            )
            cpu_init_weights = lambda: torch.device("cpu")
            actor_module = _load_actor_module_with_context(cpu_init_weights)
