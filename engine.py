...
def launch_engines(num_engines: int, model_name: str, precision: str = "bfloat16", batch_size: int = 25, tensor_parallel_size: int = 1, enable_prefix_caching: bool = False, gpu_memory_utilization: float = 0.75, multimodal: bool = False):
    ...
    """
    # Allow overriding the per-batch init size via env. Initializing many vLLM
    # engines simultaneously can race on torch.distributed's open-port pick
    # (EADDRINUSE), which shows up under multi-node Ray. A small batch serializes
    # init into waves (each wave is barriered by store_base_weights below).
    _env_bs = os.environ.get("ENGINE_BATCH_SIZE")
    if _env_bs:
        try:
            batch_size = max(1, int(_env_bs))
        except ValueError:
            pass
    ...
    for batch_idx in range(num_batches):
        ...
        engine_kwargs = dict(
            ...
            worker_extension_cls="iterative_randopt.worker_extn.WorkerExtension",
            dtype=precision,
